...
export default observer(function ServiceUser({ services, predicate, selectedService, setService }: Props) {
                    ...
                    <Grid.Column key={service.id}>
                        <Card centered={true} fluid className="service-card">
                            <Card.Content className="price-container">
                                <Card.Header style={{ fontSize: '15px' }} className="service-name">{service.name}</Card.Header>
                            ...
                            <Card.Content extra>
                                {
                                    predicate === 'admin' ?
                                        <>
                                            <Button color="blue" content='Edit' as={Link} to={`/admin/manageService/${service.id}`} />
                                            <Button color="red" content='Delete' loading={loading} onClick={() => deleteService(service.id)} />
                                        </> :
                                        <Button icon 
                                            className={selectedService === service.id ? "active" : "basic"}
                                            color="blue" onClick={() => setService(service.id, service.price)}>
                                            <span>Book Now </span>
